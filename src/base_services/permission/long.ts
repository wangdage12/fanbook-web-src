// https://github.com/google/closure-library/blob/7818ff7dc0b53555a7fb3c3427e6761e88bde3a2/closure/goog/math/long.js#L2
/**
 * @license
...
export default class Long {
  ...
  toString(opt_radix?: number) {
    ...
    const radixToPower = Long.fromBits(radixPowSafeDigits, radixPowSafeDigits / TWO_PWR_32_DBL_)
   ...
   */
  toUnsignedString(opt_radix: number) {
  ...
  negate() {
    ...
    const negHigh = (~this.high_ + (overflowFromLow ? 1 : 0)) | 0
   ...
   */
  static isStringInRange(str: string, opt_radix: number) {
    const radix = opt_radix || 10
    if (radix < 2 || 36 < radix) {
      throw new Error('radix out of range: ' + radix)
    }

    const extremeValue = str.charAt(0) == '-' ? MIN_VALUE_FOR_RADIX_[radix] : MAX_VALUE_FOR_RADIX_[radix]
...
const IntCache_ = new Map<number, Long>()
 ...
 * @private
 */
function getCachedIntValue_(value: number) {
  if (!IntCache_.has(value)) {
    IntCache_.set(value, new Long(value, value < 0 ? -1 : 0))
  }
  return IntCache_.get(value) as Long
}
 ...
 * @private @const
 */
const MAX_SAFE_INTEGER_ = Number.MAX_SAFE_INTEGER ?? 0x1fffffffffffff
