...
import { ChannelType, MediaModerationMode } from 'fb-components/struct/ChannelStruct.ts'
...
import MediaModerationModeSettings from './MediaModerationModeSettings.tsx'
...
export default function ChannelPermissionSettings({ value: channel, onChange }: ChannelSettingsSubPageProps) {
  ...
  const menu: MenuItem[] = useMemo(
    () => [
      { label: '谁可以查看', tips: '允许成员查看频道。', permission: ChannelPermission.ViewChannel },
      ...(channel.type === ChannelType.guildVoice || channel.type === ChannelType.guildVideo ?
        [
          { label: '谁可以开麦', tips: '允许成员在频道中上麦发言。', permission: AudiovisualPermission.Speak },
          {
            label: '谁可以视频和屏幕共享',
            tips: '允许成员开启摄像头视频、共享屏幕。',
            permission: AudiovisualPermission.Camera,
          },
          {
            label: '控场模式',
            tips: '允许成员关闭其他成员的麦克风、摄像头和共享屏幕，置顶其他成员窗口，维持现场秩序。',
            permission: AudiovisualPermission.MuteMembers,
            component: <MediaModerationModeSettings />,
          },
        ]
      : []),
      ...(channel.type === ChannelType.guildText ?
        [
          { label: '谁可以发言', tips: '允许成员在频道中发言。', permission: ChannelPermission.SendMessage },
          { label: '谁可以表态', tips: '允许成员添加表情符号至消息。', permission: ChannelPermission.React },
          ...(guild.upload_files ?
            [
              {
                label: '谁可以上传文件',
                tips: '允许成员在圈子或频道中上传文件。',
          ...
          {
            label: '@全体成员、@身份组',
            tips: '允许成员使用@全体成员、@某个身份组。',
            permission: ChannelPermission.MentionEveryone,
          },
          {
            label: '谁可以管理消息',
            tips: '允许成员撤回在身份组列表中低于该身份组的其其他身份组及成员发出的消息、以及pin和置顶消息。',
            permission: ChannelPermission.ManageMessages,
          },
        ]
      : []),
        ...
        [
          {
            label: '谁可以发起提问',
            tips: '允许成员发起提问。',
            permission: QuestionPermission.Question,
          },
          {
            label: '谁可以回复问题',
            tips: '允许成员评论问题。',
            permission: QuestionPermission.Answer,
          },
        ]
      : []),
    ],
...
function RightPane({
  ...
  const initialValue = useMemo(() => {
    ...
    const result = {
      ...
      ...(permission === AudiovisualPermission.MuteMembers ? { voice_control: channel.voice_control ?? MediaModerationMode.Moderator } : {}),
  ...
  function migrateFromOldPermissionSystem() {
    ...
    if ((everyone.deny & permission) === 0) {
      ...
      store.dispatch(guildListActions.updateChannel(pick(channel, 'guild_id', 'channel_id', 'overwrite')))
  ...
  const submit = async (data: T, old: T) => {
    ...
    if (data.voice_control !== old.voice_control) {
      channel.voice_control = data.voice_control!
      await ChannelAPI.update(pick(channel, ['guild_id', 'channel_id', 'voice_control']))
    }
    onChange?.(clone(channel))
  }
  return (
